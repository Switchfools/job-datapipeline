...
def extract_listings(page_soup):
    ...
    for a in page_soup.find_all('a', href=True):
        if "/partner/jobListing.htm?" in a['href']:
            ...
            key_url="www.glassdoor.com" + a['href']
            listings_list.append(key_url)
            try:
                sal=a.parent.find("span",attrs={'data-test':"detailSalary"}).getText()
                listings_dict[key_url]=sal
            except Exception as e:
                #print("[ERROR] {} in extracting salary".format(e))
                sal="NaN"
    ...
    return listings_set, jobCount, listings_dict
