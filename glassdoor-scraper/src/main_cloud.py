...
def load_configs(path, parser):
    ...
    for search_term in parser.job_type:
        for location in parser.location:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-job', '--job_type', type=str, nargs='+', default=["data-engineer"],
                        help='a search term for a job eg: data-scientist (separate search term by spaces, use - for separate words)')
    parser.add_argument('-tgn', '--target_num', type=int, default=30,
                        help='Number of jobs to serch for eg: 30')
    parser.add_argument('-loc', '--location', type=str,nargs='+', default=["colombia"],
                        help='locations to search jobs from eg: us')
    base_urls, target_num,search_terms_dict = load_configs(path="./data/config.json",parser=parser.parse_args())
